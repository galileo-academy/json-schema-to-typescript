...
function generate(ast, options) {
    ...
    var result = declareNamedTypes(ast, options, ast.standaloneName);
    var interfaces = declareNamedInterfaces(ast, options, ast.standaloneName);
...
function declareNamedInterfaces(ast, options, rootASTName, processed) {
    ...
    switch (ast.type) {
        case 'ARRAY':
            ...
            break;
        case 'INTERFACE':
            var val0 = (0, AST_1.hasStandaloneName)(ast) &&
                (ast.standaloneName === rootASTName || options.declareExternallyReferenced) &&
                generateStandaloneInterface(ast, options);
            var val1 = getSuperTypesAndParams(ast)
    ...
    switch (ast.type) {
        ...
        case 'ARRAY':
            return (function () {
                ...
                var result = type.endsWith('"') ? '(' + type + ')[]' : type + '[]';
                return "".concat(ast.readonly ? 'readonly ' : '').concat(result);
            })();
        case 'BOOLEAN':
            return 'boolean';
        case 'INTERFACE':
...
function escapeKeyName(keyName, ast) {
    var _a;
    var readonly = Object.keys(ast).includes('readonly') &&
        ((_a = Object.entries(ast).find(function (_a, index, obj) {
            var key = _a[0], value = _a[1];
            return key === 'readonly';
        })) === null || _a === void 0 ? void 0 : _a[1]);
    if (keyName.length && /[A-Za-z_$]/.test(keyName.charAt(0)) && /^[\w$]+$/.test(keyName)) {
        return "".concat(readonly ? 'readonly ' : '').concat(keyName);
    }
    if (keyName === '[k: string]') {
        return keyName;
    }
